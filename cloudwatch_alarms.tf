locals {
    ...
    error_endpoints     = distinct(compact(concat(list(local.sns_error_topic_arn), var.additional_error_endpoint_arns)))
    info_endpoints      = distinct(compact(concat(list(local.sns_info_topic_arn), var.additional_info_endpoint_arns)))
